...
ANY_array = LTR_array + DNA_array + HEL_array
...
for seq_record in SeqIO.parse(ntpath.basename(args.mfilename), 'fasta'):  # (generator)
    # print(seq_record.id)
    print("Analyzing: {}".format(seq_record.id))
    for blast_record in open(ntpath.basename(args.mfilename) +'_blastout.txt').readlines():  # Will close handle cleanly
        df = (pd.DataFrame(blast_record.split('\t')))
        # print(df[0][0])
        if "#Unknown" in df[0][0] and df[0][0] == seq_record.id and any(c in df[0][2] for c in LTR_array) == True:
            count_LTR+=1
            print(">" + seq_record.id.split("#")[0] + "#LTR", file=sample)
            print(seq_record.seq, file=sample)
        elif "#Unknown" in df[0][0] and df[0][0] == seq_record.id and any(d in df[0][2] for d in DNA_array)  == True:
            count_DNA+=1
            print(">" + seq_record.id.split("#")[0] + "#DNA", file=sample)
            print(seq_record.seq, file=sample)
        elif "#Unknown" in df[0][0] and df[0][0] == seq_record.id and any(e in df[0][2] for e in HEL_array)  == True:
            count_Helitron+=1
            print(">" + seq_record.id.split("#")[0]  + "#RC/Helitron", file=sample)
            print(seq_record.seq, file=sample)
        elif "#Unknown" in df[0][0] and df[0][0] == seq_record.id and not any (e in df[0][2] for e in ANY_array)  == True:
            count_Unknown+=1
